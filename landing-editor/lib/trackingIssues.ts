...
export function collectTrackingIssues(draft: LandingPageDraft, t: IssuesDict = defaultIssuesDict): string[] {
  ...
  for (const p of pixels) {
    ...
    if (/\s/.test(p.id)) out.push(t.pixelIdWhitespace(PROVIDER_LABEL[p.provider]));
  }
  return [...new Set(out)];
}
