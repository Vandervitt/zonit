      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-lg border border-edge bg-panel p-3 shadow-xl">
          <div className="flex items-start justify-between gap-2">
            <p className="text-sm font-medium text-amber-700">{t.title(issues.length)}</p>
            <button onClick={() => setOpen(false)} className="shrink-0 text-xs text-ink-muted hover:text-ink">{t.close}</button>
          </div>
