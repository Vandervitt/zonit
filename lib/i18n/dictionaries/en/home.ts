...
export const home = {
  ...
  // 自检器的首页次级入口。刻意做成归因区尾部的一行小字，不是独立区块、更不是首屏 CTA：
  // 「自检 / 体检」对不投放的访客是个陌生动作，放大了会把首页从「我能帮你拿到线索」
  // 偏移成「我来挑你的毛病」，反而窄化受众。放在归因区之后，读到这里的人已经在关心
  // 「我那张页现在到底怎么样」，人群自然筛过一遍。
  // 措辞同样受自检器红线约束：不承诺过审、不给评分（见 dictionaries/*/tools.ts）。
  pageCheckTeaser: {
    text: "Already running a landing page somewhere else?",
    link: "See what ad reviewers look at on it",
  },
