// lib/leads/dial-codes.ts
// 国际电话国码表（ITU-T E.164 国家代码）。
//
// 为什么存在：访客手填的号码普遍省略国码（本地人填本地号从不写国码），存进库就
// 永远拼不出 wa.me 链接，客户也未必打得通。故在表单侧按访客 IP 预选国码并强制携带，
// 让号码进库即 E.164——把问题挡在写入时，而不是读取时猜。
//
// 首屏预算：本表体积不小，**禁止在落地页客户端组件里静态 import**。
// 服务端（app/p/[slug]/page.tsx）静态 import 只进服务端 bundle，不影响首屏；
// 客户端一律走 LeadForm 里的 `import()`，落到独立 chunk 且在 idle 时才取。

export interface DialCode {
  /** ISO 3166-1 alpha-2，作为选择器的 value（多个国家共用同一国码，故不能用国码当 value）。 */
  iso: string;
  /** 含 `+` 前缀的国码。 */
  dial: string;
  /** 英文国名——落地页面向海外访客，且模板详情页会公开渲染，不得出现中文。 */
  name: string;
}

/** 未知/无法判定访客国家时的兜底（海外投放主战场）。 */
...
export const DIAL_CODES: DialCode[] = [
  ...
  { iso: "KN", dial: "+1", name: "St. Kitts and Nevis" },
  { iso: "LC", dial: "+1", name: "St. Lucia" },
  { iso: "VC", dial: "+1", name: "St. Vincent and the Grenadines" },
...
/** ISO 国家码 → 表项；未知（含 Vercel geo 头缺失）回退到默认国。 */
export function dialCodeFor(iso: string | null | undefined): DialCode {
  return BY_ISO.get((iso ?? "").toUpperCase()) ?? BY_ISO.get(DEFAULT_DIAL_ISO)!;
}
