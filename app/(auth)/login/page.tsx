...
export default function LoginPage() {
  ...
  return (
    <div className="w-full max-w-sm bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl shadow-bloom-500/10 border border-bloom-100 p-8">
      ...
      <div className="flex flex-col gap-3 mb-6">
        <button
          ...
          className="w-full flex items-center justify-center gap-3 border border-bloom-200 rounded-xl py-2.5 text-sm text-foreground/80 hover:bg-bloom-50 hover:border-bloom-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {oauthLoading === AuthProvider.Google ? (
            <div className="w-4 h-4 border-2 border-bloom-200 border-t-bloom-500 rounded-full animate-spin" />
          ) : (
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="flex-1 h-px bg-bloom-100" />
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <div className="relative">
          <input
            ...
            required
            className="w-full px-4 py-2.5 rounded-xl border border-bloom-200 bg-white/60 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-bloom-400 focus:ring-2 focus:ring-bloom-200 transition-colors"
          />
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-2.5 mt-2 rounded-xl bg-gradient-to-r from-bloom-500 to-tech text-white text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-60 shadow-lg shadow-bloom-500/30"
        >
      ...
      <p className="text-xs text-muted-foreground text-center mt-5">
        No account?{" "}
        <Link href={Routes.Register} className="text-bloom-600 font-medium hover:underline">
          Sign up
        </Link>
      </p>

      {process.env.NODE_ENV === "development" && (
        <div className="mt-4 pt-4 border-t border-dashed border-bloom-100">
          <button
            type="button"
