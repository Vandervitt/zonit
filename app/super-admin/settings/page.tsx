...
async function getSuperAdmins(): Promise<SuperAdminRow[]> {
  const result = await pool.query(
    `SELECT id, email, plan, comp_plan, disabled_at FROM users WHERE role = $1 ORDER BY email`,
    [UserRole.SUPER_ADMIN],
  );
  return result.rows.map((u) => ({
    id: u.id as string,
    email: u.email as string,
    effectivePlan: effectivePlan((u.plan ?? "free") as PlanId, u.comp_plan as PlanId | null),
