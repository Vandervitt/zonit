...
| 单元 | `pnpm test` | ✅ **495 passed / 75 files** |
| Lint | `pnpm lint` | ✅ 0 error（5 warning 为既有） |
| E2E | `pnpm test:e2e e2e/i18n.spec.ts` | ✅ **21 passed** |
...
| 项 | 内容 |
| --- | --- |
| 内容拆分 | 两份文档的正文移入 `components/marketing/legal-content/{en,zh}.ts`，页面收敛为薄壳 |
| 隐私政策 | 13 节 → 14 节（新增「语言版本」） |
| 服务条款 | 15 节 → 16 节（新增「语言版本」） |
| 语言条款 | **两版均声明英文版为准据文本**，歧义时以英文版为准 |
| 最后更新 | `2026 年 7 月 20 日` → `2026 年 7 月 28 日`（新增条款属实质性变更，日期须同步） |
| metadata | 由裸 `Metadata` 改走 `marketingMetadata()`——这两页改造前**没有 canonical，也没有 hreflang**，现已补齐 |

## ⚠️ 需要法律审阅（未完成，如实记录）

英文版被指定为准据文本，意味着**具约束力的是我翻译的这一版**。我不是律师，这两份文档的英文表述未经法律专业审阅。以下几点尤其建议在正式对外前请律师过一遍：

- 责任限制与免责声明的措辞（第 10、11 节）——各法域对免责范围的强制性限制不同
- 数据控制者 / 处理方的角色划分（隐私政策第 5 节）——GDPR 语境下用词有特定含义
- 「本条款未指定特定的排他管辖司法辖区」与新增的准据语言条款并存时的实际效果

本 PR 交付的是**工程上的双语一致性**，不构成法律意见。

## 人工走查

| 检查项 | 结果 |
| --- | --- |
| `/privacy` `/zh/privacy` `/terms` `/zh/terms` 均 200 | ✅ |
| 英文版含 “the English version prevails” | ✅ |
| 中文版含「以英文版为准」 | ✅ |
| 法务页 hreflang（改造前无） | ✅ x-default / en / zh-Hans 三条齐全 |
