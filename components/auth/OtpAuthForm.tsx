 ...
 */
export function OtpAuthForm({ locale, token }: { locale: Locale; token?: string | null }) {
  const t = getDictionary(locale).auth.otp;
  ...
  const verify = useMutation(
    async (args: { email: string; code: string }) => {
      ...
      if (res?.error) throw new ApiError(401, t.invalidCode);
      return res;
    },
    {
      errorToast: false,
  ...
  if (phase === "email") {
    return (
      <form onSubmit={handleSendCode} className="flex flex-col gap-3">
        ...
        >
          {sendCode.isMutating ? t.sending : t.sendCode}
        </button>
      </form>
    );
  }
  ...
  return (
    <form onSubmit={handleVerify} className="flex flex-col gap-3">
      ...
      >
        {verify.isMutating ? t.verifying : t.submit}
      </button>
      <div className="flex items-center justify-between text-xs px-1">
        ...
        >
          {cooldown > 0 ? t.resendCooldown.replace("{seconds}", String(cooldown)) : t.resend}
        </button>
      </div>
    </form>
  );
